    ...
    public List<Double> averageOfLevels(TreeNode root) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                sum = sum.add(new BigDecimal(node.val));
            ...
            res.add(sum.divide(new BigDecimal(size), 5, RoundingMode.CEILING).doubleValue());
        }
...
class Main {
    ...
    public static void main(String[] args) {
        cases().forEach(c -> {
            List<Double> res = new Solution().averageOfLevels(c.root);
    ...
    private static List<Case> cases() {
        ...
        List<Double> e1 = Arrays.asList(3.00000, 14.50000, 11.00000);
...
class Case {
    ...
    public Case(TreeNode root, List<Double> expected) {
        this.root = root;
